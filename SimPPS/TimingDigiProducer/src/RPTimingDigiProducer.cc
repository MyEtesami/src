#include "SimPPS/TimingDigiProducer/interface/RPTimingDigiProducer.h"
...
RPTimingDigiProducer::RPTimingDigiProducer(const edm::ParameterSet& conf) :
	conf_(conf) {
...
//	produces<edm::DetSetVector<RPStripDigi> > ();
        produces<edm::DetSetVector<RPLBarDigi> > ();
 	//produces<edm::DetSetVector<RPDetTrigger> > ();
        produces<edm::DetSetVector<RPTimingTrigger> > ();

	
        RP_hit_containers_.clear();
	RP_hit_containers_ = conf.getParameter<std::vector<std::string> > ("ROUList");
        std::cout<<"RP_hit_containers_ size is:"<<RP_hit_containers_.size()<<std::endl;
	verbosity_ = conf.getParameter<int> ("RPVerbosity");
        std::cout<<"verbosity_ is: "<<verbosity_<<std::endl;
	...
	rndEngine = &(rng->getEngine());
        ...
        std::cout<<"rndEngine is: "<<rndEngine<<std::endl;
...
RPTimingDigiProducer::~RPTimingDigiProducer() {
...
void RPTimingDigiProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
	using namespace edm;
	...
	std::auto_ptr<MixCollection<PSimHit> > allRPHits(
	        new MixCollection<PSimHit> (cf.product(), std::pair<int, int>(0, 0)));
                ...
                MixCollection<PSimHit>::iterator cfi;
                int count = 0;
                for (cfi = allRPHits->begin(); cfi != allRPHits->end(); cfi++) {
                        std::cout << " Hit " << count << " has tof " << cfi->timeOfFlight() << " trackid "
                                << cfi->trackId() << " bunchcr " << cfi.bunch() << " trigger " << cfi.getTrigger()
                                << ", from EncodedEventId: " << cfi->eventId().bunchCrossing() << " "
                                << cfi->eventId().event() << " bcr from MixCol " << cfi.bunch() << std::endl;
...
std::cout<<"SimHitMap.size() "<<SimHitMap.size()<<std::endl;
	...
	//	edm::DetSet<RPStripDigi> digi_collector(it->first);
                edm::DetSet<RPLBarDigi> digi_collector(it->first);
//		edm::DetSet<RPDetTrigger> trigger_collector(it->first);
                edm::DetSet<RPTimingTrigger> trigger_collector(it->first);
                std::cout<<"it->first: "<<(it->first)<<std::endl;
                std::cout<<"theAlgoMap.find(it->first)->first is: "<<theAlgoMap.find(it->first)->first<<std::endl;
		if (theAlgoMap.find(it->first) == theAlgoMap.end()) {
			theAlgoMap[it->first] = boost::shared_ptr<RPDetDigitizer>(
			      new RPDetDigitizer(conf_, *rndEngine, it->first));
		}
                std::cout<<"theAlgoMap.find(it->first)->first is: "<<theAlgoMap.find(it->first)->first<<std::endl;
		std::vector<int> input_links;
		SimRP::DigiPrimaryMapType output_digi_links;
		SimRP::TriggerPrimaryMapType output_trig_links;
                std::cout<<"SimHitMap[it->first]: "<<(SimHitMap[it->first]).size()<<std::endl;
		...
		std::vector<RPLBarDigi>::iterator stripIterator = digi_collector.data.begin();
...
cout<<"   digi_collector.data.size()  " << digi_collector.data.size() <<endl;
cout<<"   trigger_collector.data.size()  " << trigger_collector.data.size() <<endl;
...
//
	std::auto_ptr<edm::DetSetVector<RPLBarDigi> > digi_output(
	        new edm::DetSetVector<RPLBarDigi>(theDigiVector));
//	std::auto_ptr<edm::DetSetVector<RPDetTrigger> > trigger_output(
//	        new edm::DetSetVector<RPDetTrigger>(theTriggerVector));

        std::auto_ptr<edm::DetSetVector<RPTimingTrigger> > trigger_output(
                new edm::DetSetVector<RPTimingTrigger>(theTriggerVector));
...
void RPTimingDigiProducer::beginRun(edm::Run&, edm::EventSetup const& es){
...
void RPTimingDigiProducer::endJob() {
}

DEFINE_FWK_MODULE( RPTimingDigiProducer);
